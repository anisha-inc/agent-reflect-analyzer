...
def test_run_pipeline_empty_bucket_returns_baseline_summary():
    ...
    with patch("analyzer.duckdb_query.connect") as conn, \
         patch("analyzer.duckdb_query.load_sessions", return_value=[]):
        conn.return_value.close = lambda: None
        summary = run.run_pipeline(
            record=record, since="7d", limit=10, repo=None,
            emit_issues=False, top_k=10, strategy="hybrid",
            model_top_k="opus", model_rest="haiku", concurrency=3,
            cluster=False, include_closed_since=None,
            api_key_fallback=False, verbose=False,
        )
...
def test_run_pipeline_dry_run_does_not_call_emit():
    ...
    fake_candidate = Candidate(
        source="opus_top_k", pattern_id="x", title="t", frequency=2,
        symptom="s", proposed_fix="f",
    )
    with patch("analyzer.duckdb_query.connect") as conn, \
         patch("analyzer.duckdb_query.load_sessions",
               return_value=[{"sessionId": "s1"}]), \
         patch("analyzer.duckdb_query.load_events_for_sessions",
               return_value={"s1": []}), \
...
def test_run_pipeline_emit_issues_calls_gh():
    ...
    with patch("analyzer.duckdb_query.connect") as conn, \
        ...
        conn.return_value.close = lambda: None
        summary = run.run_pipeline(
            record=record, since="7d", limit=10, repo="a/b",
            emit_issues=True, top_k=10, strategy="hybrid",
            model_top_k="opus", model_rest="haiku", concurrency=3,
            cluster=False, include_closed_since=None,
            api_key_fallback=False, verbose=False,
        )
